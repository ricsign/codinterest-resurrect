{{--layui--}}
<link rel="stylesheet" href="{{asset('layui/css/layui.css')}}">
{{--markdown--}}
<link rel="stylesheet" href="{{url('https://stackedit.io/style.css')}}" />
{{--bootstrap--}}
<link rel="stylesheet" href="{{url('https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css')}}" integrity="sha384-JcKb8q3iqJ61gNV9KGb8thSsNjpSL0n8PARn9HuZOnIxN0hoP+VmmDGMN5t9UJ0Z" crossorigin="anonymous">
{{--icon--}}
<link rel="icon" href="{{asset('imgs/site/icon.png')}}">
<link rel="stylesheet" href="{{asset('styles/root.css')}}">
{{--font--}}
<link href="{{url('https://fonts.googleapis.com/css2?family=Lato:ital,wght@0,300;0,400;0,700;1,300;1,400;1,700&display=swap')}}" rel="stylesheet">
<link href="{{url('https://fonts.googleapis.com/css2?family=Lobster&display=swap')}}" rel="stylesheet">
<link href="{{url('https://fonts.googleapis.com/css2?family=Fredoka+One&display=swap" rel="stylesheet')}}" rel="stylesheet">
{{--root--}}
<link rel="stylesheet" href="{{\Illuminate\Support\Facades\URL::asset('styles/root.css')}}">
